...
document.addEventListener('DOMContentLoaded', textrareaCounter);
document.addEventListener('turbo:load', textrareaCounter);
    ...
    if (sluggableInputs.length > 0) {
        sluggableInputs.forEach(item => {
            ...
            item.addEventListener('input', function () {
                ...
                field.value = slugInput.value;
                for(let input of item.closest('fieldset').querySelectorAll('input')){
...
function textrareaCounter() {
    for (let textarea of document.querySelectorAll('textarea')) {
        textarea.addEventListener('input', function () {
            if(textarea.parentElement.querySelector('span.counter')){
                textarea.parentElement.querySelector('span.counter').remove()
            }
            let element = document.createElement('span');
            element.style.marginLeft = '60%';
            element.classList.add('counter');
            element.innerText = textarea.value.length + ' symbols';
